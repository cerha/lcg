...
class HtmlExporter(Exporter):
    ...
    def __init__(self, stylesheet=None, inlinestyles=False):
        ...
        self._formatter = HtmlMarkupFormatter()
    ...
    def _output_file(self, node):
        """Return full pathname of node's output file relative to export dir."""
        # TODO: This should be really done here, but some more code has to be
        # rewritten.
        return node.output_file()

    def _url(self, target, relative=False):
...
class HtmlStaticExporter(HtmlExporter, FileExporter):
    ...
    def _body_parts(self, node):
        parts = super(HtmlStaticExporter, self)._body_parts(node)
